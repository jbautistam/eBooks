	...
	public class ePubCreator
	{
		...
		public void Create(string fileName, ePubEBook eBook)
		{
			...
			Compressor objCompressor = new Compressor();
				...
				objCompressor.Compress(fileName, pathTemporal, Compressor.CompressType.Zip);
		...
		private MLNode GetMetadata(OPFPackage package)
		{
			MLNode mlNode = new MLNode(OPFPackageConstants.TagMetadata);
				...
				mlNode.Nodes.Add(Extensions.DC.DublinCoreConstants.NameSpacePrefix,
								 ...
								 ConvertDate(package.Metadata.DateOriginalPublished));
				mlNode.Nodes.Add(Extensions.DC.DublinCoreConstants.NameSpacePrefix,
								 Extensions.DC.DublinCoreConstants.TagDate,
								 ConvertDate(package.Metadata.DatePublished));
				mlNode.Nodes.Add(Extensions.DC.DublinCoreConstants.NameSpacePrefix,
			...
			MLNodesCollection mlNodes = new MLNodesCollection();
				...
				foreach (NavPoint navPoint in navPoints)
				{
					...
					MLNode contentML;
						...
						// Añade la etiqueta
						navPointML.Nodes.Add(NCXConstants.TagNavPointLabel).Nodes.Add(NCXConstants.TagText,
																					  navPoint.Title);
						// Añade el contenido
						contentML = navPointML.Nodes.Add(NCXConstants.TagNavPointContent);
						contentML.Attributes.Add(NCXConstants.TagNavPointContentSrc, Path.GetFileName(navPoint.URL));
						// Añade los hijos
						navPointML.Nodes.Add(GetNodesNavPoint(navPoint.Pages));
						// Añade el nodo a la colección
						mlNodes.Add(navPointML);
				}
